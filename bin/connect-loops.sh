...
for ((i = 1; i <= $AMOUNT ; i++ )); do
    ...
    [[ "$RNDHEAD" =~ $RX ]] && START="${BASH_REMATCH[1]}" && PASS1="${BASH_REMATCH[2]}" && MID="${BASH_REMATCH[3]}"
    echo $RNDHEAD : $START / $PASS1 / $MID

    RNDPATH=`ls $SRCDIR/$MID-*.mp4 |sort -R | tail -1`
    RNDTAIL=`basename $RNDPATH`
    [[ "$RNDTAIL" =~ $RX ]] && PASS2="${BASH_REMATCH[2]}" && END="${BASH_REMATCH[3]}"
    echo $RNDTAIL: $MID / $PASS2 / $END

    OUTFILE=$START-$PASS1$MID-$PASS2$END.mp4 
